...
import { MessageSquare, Database, Settings, Send, User, LogOut, ChevronRight, Activity, Zap, Sparkles, Shield, Cpu, Code2, Trash2, Plus, MessageCircle, Loader2, Copy, Brain } from 'lucide-react';
    ...
    return (
        <div className="flex h-screen bg-dark-bg text-slate-100 overflow-hidden font-sans">
            ...
            <aside className="w-72 glass border-r border-white/5 flex flex-col z-20">
                <div className="p-6 border-b border-white/5 cursor-pointer" onClick={() => { setSelectedSector(null); setShowWelcome(true); }}>
                    ...
                    <div className="flex items-center justify-between mt-2">
                        ...
                        {user?.role === 'admin' && (
                            <Link to="/admin" className="text-[10px] font-black bg-amber-500/10 text-amber-500 px-2 py-0.5 rounded-sm uppercase tracking-widest hover:bg-amber-500 hover:text-white transition-all flex items-center gap-1">
                    ...
                    {showWelcome ? (
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-hidden flex flex-col relative">
                                                ...
                                                {msg.role === 'user' ? (
                                                    ...
                                                    <div className="space-y-4">
                                                        {msg.role === 'error' ? (
                                                            ...
                                                            <div className="text-[15px] leading-relaxed text-blue-50/80">
                                                                {msg.content.includes('```sql') ? (
                                                                    <>
                                                                        {/* Texto antes do código */}
                                                                        {msg.content.split('```sql')[0].trim().length > 0 && (
                                                                            <p className="mb-4">{msg.content.split('```sql')[0].trim()}</p>
                                                                        )}
                                                                        ...
                                                                        {msg.content.split('```')[2] && msg.content.split('```')[2].trim().length > 0 && (
                                                                            <p className="mt-4">{msg.content.split('```')[2].trim()}</p>
                                                                        )}
                                                                    </>
                                                                ) : (
                                                                    <p>{msg.content}</p>
                                ...
                                <div className="p-8 pb-20 bg-gradient-to-t from-dark-bg via-dark-bg to-transparent relative z-30">
